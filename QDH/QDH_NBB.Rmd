...
g1 <- pokemon %>% 
  ggplot(aes(x = HP)) +
  geom_histogram(bins = 22, fill = colorsKids[5], alpha = 0.6, color = colorsKids[1]) +
geom_vline(aes(xintercept=mean(HP, na.rm=T)), color="red", linetype="dashed", size=1) +
  ggtitle("Pokemon Hit Point Distribution") +
  xlab("Pokemon HP") +  
  ylab("Pokemon HP Count")  
g1
ggsave(file="g1.svg")
```  
